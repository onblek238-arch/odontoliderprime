...
function confirmationWhatsappUrl(app: Appointment) {
  ...
  const message = encodeURIComponent(
    `Olá, ${firstName}! Sua avaliação gratuita na Sorriso Prime Odontologia foi confirmada na ${unit}, para ${date} às ${time}.\n\n` +
    ...
    <div className="admin-shell">
      <header className="admin-topbar">
        <div className="admin-brand">
          ...
          <div>
            <strong>Sorriso Admin Prime</strong>
            ...
            {filteredAppointments.map((app) => {
              ...
              return (
                <motion.article
                  ...
                  className={`admin-appointment-card ${expandedAppointmentId === app.id ? 'is-expanded' : ''}`}
                >
                  <button
                    type="button"
                    className="admin-mobile-row"
                    onClick={() => setExpandedAppointmentId((current) => (current === app.id ? null : app.id))}
                  >
                    <div>
                    ...
                    <em className={statusClass(app.status)}>{statusLabel(app.status)}</em>
                  </button>
